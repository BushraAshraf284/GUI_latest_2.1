...
public class ObstacleSpawnerRocket : MonoBehaviour
{
    ...
    void SpawnObstacle ()
    {
        ...
        GameObject obstacle = Instantiate(obstacles[Random.Range(0, 6)], GetSpawnPosition(), Quaternion.identity);

        // set obstacle's direction to move in
        obstacle.GetComponent<ObstacleRocket>().moveDir = new Vector3(obstacle.transform.position.x > 0 ? -1 : 1, 0, 0);
    }
